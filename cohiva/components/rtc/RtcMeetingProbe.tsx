...
type IndividualPermissions = Record<
  string,
...
export default function RtcMeetingProbe() {
  const [roomId, setRoomId] = useState("cohiva-rtc-test");
  const [status, setStatus] = useState<
    "idle" | "connecting" | "reconnecting" | "joined" | "ended" | "error"
  >("idle");
  ...
  const resetMediaForReconnect = useCallback(() => {
    for (const pending of pendingRef.current.values()) {
      pending.reject(new Error("RTC connection was interrupted."));
    }
  ...
  const scheduleReconnect = useCallback(() => {
    if (manualDisconnectRef.current || reconnectTimerRef.current !== null) return;

    reconnectAttemptRef.current += 1;
    const delay = Math.min(8_000, 1_000 * 2 ** Math.min(3, reconnectAttemptRef.current - 1));
    setStatus("reconnecting");
    addLog(`Connection interrupted. Reconnecting in ${Math.round(delay / 1000)}s...`);
  ...
  const disconnect = useCallback(async () => {
    ...
    setPermissions(DEFAULT_ROOM_PERMISSIONS);
  ...
  const connect = useCallback(async (isReconnect = false) => {
    if (!isReconnect && (status === "connecting" || status === "joined")) return;
    ...
    addLog(isReconnect ? "Requesting a fresh RTC token for reconnection..." : "Requesting a Cohiva RTC token...");
    ...
    try {
      ...
      if (payload.userId) setSelfUserId(payload.userId);
      if (payload.role) setSelfRole(payload.role);
      ...
      socket.addEventListener("message", async (event) => {
        ...
        if (message.event === "cohiva.media-disabled") {
          ...
          addLog(`The host disabled your ${control}.`);
          return;
        }

        if (message.event === "cohiva.permissions-updated") {
          if (message.data?.permissions) {
            setPermissions((current) => ({ ...current, ...message.data.permissions }));
          }
          if (message.data?.individualPermissions) {
            setIndividualPermissions(message.data.individualPermissions);
          }
          addLog("Meeting permissions were updated.");
          return;
        }
        ...
        if (message.event === "cohiva.kicked") {
          manualDisconnectRef.current = true;
          setStatus("error");
          addLog(message.data?.blocked ? "You were blocked by the host." : "You were removed by the host.");
        ...
        if (message.event === "call.ended") {
          manualDisconnectRef.current = true;
          setStatus("ended");
          addLog("The host ended the meeting for everyone.");
      ...
      socket.addEventListener("open", async () => {
        try {
          ...
          if (joined.selfRole === "host" || joined.selfRole === "participant") {
            setSelfRole(joined.selfRole);
          }
          if (joined.permissions) {
            setPermissions((current) => ({ ...current, ...joined.permissions }));
          }
          if (joined.individualPermissions) {
            setIndividualPermissions(joined.individualPermissions);
          }
          ...
          setStatus("joined");
          reconnectAttemptRef.current = 0;
          addLog(`${isReconnect ? "Rejoined" : "Joined"}. ${nextParticipants.length} RTC participant(s).`);

          if (isReconnect) {
            if (desiredMicRef.current) await startMicrophone();
            if (desiredCameraRef.current) await startCamera();
            if (desiredScreenRef.current) {
              desiredScreenRef.current = false;
              addLog("Screen sharing must be restarted manually after reconnecting.");
            }
          }
        } catch (error) {
          setStatus("error");
      ...
      socket.addEventListener("close", (event) => {
        ...
        if (manualDisconnectRef.current || event.code === 4000 || event.code === 4001) {
          return;
        }
      ...
      if (isReconnect && !manualDisconnectRef.current) {
        scheduleReconnect();
      } else {
        setStatus("error");
      }
  ...
  useEffect(() => {
    const onOffline = () => {
      if (status === "joined") {
        addLog("Browser is offline. Waiting for network recovery...");
        setStatus("reconnecting");
      }
    };
    ...
    const onOnline = () => {
      ...
      if (!socket || socket.readyState !== WebSocket.OPEN) {
        ...
        addLog("Network is back. Reconnecting now...");
        connectRef.current(true);
      }
    };
  ...
  const moderateParticipant = useCallback(
    async (targetUserId: string, control: "audio" | "video" | "screenshare" | "kick" | "block") => {
      try {
        await request("moderate", { targetUserId, control });
        addLog(`Host moderation sent: ${control}.`);
      } catch (error) {
        addLog(error instanceof Error ? error.message : "Moderation failed.");
      }
    },
    [addLog, request]
  );
  ...
  const updateRoomPermission = useCallback(
    async (field: keyof RoomPermissions, allowed: boolean) => {
      try {
        const result = await request("setRoomPermission", { field, allowed });
        if (result?.permissions) setPermissions(result.permissions);
        if (result?.individualPermissions) {
          setIndividualPermissions(result.individualPermissions);
        }
      } catch (error) {
        addLog(error instanceof Error ? error.message : "Unable to update permission.");
      }
    },
    [addLog, request]
  );
  ...
  const endMeeting = useCallback(async () => {
    try {
      ...
      addLog(error instanceof Error ? error.message : "Unable to end meeting.");
    }
  }, [addLog, request]);
  ...
  const selfIndividual = individualPermissions[selfUserId] ?? {};
  const canUseMic =
    selfRole === "host" || (permissions.studentMic !== false && selfIndividual.audio !== false);
  const canUseCamera =
    selfRole === "host" || (permissions.studentCamera !== false && selfIndividual.video !== false);
  const canShareScreen =
    selfRole === "host" ||
    (permissions.studentScreenShare !== false && selfIndividual.screenShare !== false);
    ...
    <main className="mx-auto flex min-h-[calc(100vh-90px)] w-full max-w-6xl flex-col gap-6 px-4 py-8 text-[#3B3732] sm:px-6">
      <section className="rounded-[28px] border border-[#3B3732]/10 bg-[#FFF9EF] p-6 shadow-xl sm:p-8">
        <p className="text-xs font-black uppercase tracking-[0.22em] text-[#A2AB73]">
          Phase 5 · resilience + moderation
        </p>
        <h1 className="mt-2 text-3xl font-black">Cohiva RTC media test</h1>
        <p className="mt-2 max-w-3xl text-sm text-[#6E655B]">
          This page now tests Cohiva-owned media, automatic reconnection, host moderation,
          and classroom media permissions. Normal meeting pages still use Stream for now.
        </p>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <input
            ...
            disabled={status === "connecting" || status === "reconnecting" || status === "joined"}
          ...
          {status === "joined" ? (
            ...
            >
              {status === "connecting" ? "Connecting..." : status === "reconnecting" ? "Reconnecting..." : "Connect RTC"}
            </button>
          )}
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm font-bold">
            {status === "joined"
              ? `Cohiva RTC ready · ${selfRole === "host" ? "Host" : "Participant"}`
              : status === "ended"
                ? "Meeting ended"
                : status === "error"
                  ? "RTC test encountered an error"
                  : status === "reconnecting"
                    ? "Reconnecting to Cohiva RTC..."
                    : status === "connecting"
          ...
          {status === "joined" && (
            <div className="ml-auto flex flex-wrap gap-2">
              <button
                ...
                disabled={!micOn && !canUseMic}
                className={`rounded-xl px-4 py-2 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-45 ${
                  micOn
              ...
              <button
                ...
                disabled={!cameraOn && !canUseCamera}
                className={`rounded-xl px-4 py-2 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-45 ${
                  cameraOn
                    ? "bg-emerald-100 text-emerald-800"
                    : "bg-[#EFE7DB] text-[#3B3732]"
      ...
      {status === "joined" && selfRole === "host" && (
        <section className="rounded-[24px] border border-[#CF3864]/15 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-[#CF3864]">Host controls</p>
              <h2 className="mt-1 text-xl font-black">Classroom media permissions</h2>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            {[
              ["studentMic", "Student microphones", permissions.studentMic],
              ["studentCamera", "Student cameras", permissions.studentCamera],
              ["studentScreenShare", "Student screen sharing", permissions.studentScreenShare],
            ].map(([field, label, allowed]) => (
              <button
                key={String(field)}
                type="button"
                onClick={() => void updateRoomPermission(field as keyof RoomPermissions, !allowed)}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="font-bold">
                        ...
                        {participant.userId === selfUserId ? " · You" : ""}
                      </p>
                      <p className="truncate text-xs text-[#7A7067]">{participant.userId}</p>
                    </div>
                  </div>

                  {selfRole === "host" && participant.userId !== selfUserId && !participant.isHost && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button type="button" onClick={() => void moderateParticipant(participant.userId, "audio")} className="rounded-lg bg-white px-2.5 py-1.5 text-xs font-bold">Mute mic</button>
                      <button type="button" onClick={() => void moderateParticipant(participant.userId, "video")} className="rounded-lg bg-white px-2.5 py-1.5 text-xs font-bold">Camera off</button>
                      <button type="button" onClick={() => void moderateParticipant(participant.userId, "screenshare")} className="rounded-lg bg-white px-2.5 py-1.5 text-xs font-bold">Stop share</button>
                      <button type="button" onClick={() => void moderateParticipant(participant.userId, "kick")} className="rounded-lg bg-amber-100 px-2.5 py-1.5 text-xs font-bold text-amber-900">Remove</button>
                      <button type="button" onClick={() => void moderateParticipant(participant.userId, "block")} className="rounded-lg bg-red-100 px-2.5 py-1.5 text-xs font-bold text-red-800">Block</button>
                    </div>
                  )}
                </div>
              ))
